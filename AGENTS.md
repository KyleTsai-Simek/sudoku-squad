...
This file is read by Codex (and other AI coding agents) at the start of every session. It encodes how to work in this repo so the docs stay accurate and the codebase stays coherent.
...
### Realtime subscriptions stay scoped per room
- The original one-channel plan was replaced with Supabase `postgres_changes` subscriptions filtered by room.
- Current channels: `room_players:{room_id}`, `moves:{room_id}`, and `rooms:{room_id}`.
- If presence cursors ship, add the smallest presence surface that fits coop cursors rather than introducing a second game-state transport.
