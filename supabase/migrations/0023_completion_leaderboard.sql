-- ============================================================================
-- 0023_completion_leaderboard.sql
--
-- Read model for the first global leaderboard: players ranked by unique puzzle
-- completions. The function is intentionally paged and also includes the
-- caller's own ranked row when it sits outside the requested page, so clients
-- can show "top N" plus "your place" without loading every ranked player.
-- Future leaderboard variants should add to this RPC contract (or supersede it)
-- rather than exposing raw completion rows to clients.
-- ============================================================================

create index if not exists player_completions_leaderboard_idx
  on public.player_completions (player_id, completed_at);
...
declare
  uid uuid := auth.uid();
  page_limit integer := least(greatest(coalesce(p_limit, 25), 1), 100);
...
comment on function public.get_completion_leaderboard(integer, integer, text) is
  'Paged leaderboard read model. Currently supports total_completions and includes the caller row when ranked outside the requested page.';
