...
- A daily solve is recorded only when the player completes the assigned puzzle on its assigned Pacific day; the server infers that same-day daily credit from the active `daily_puzzles` assignment, while the normal solved-game row is stored identically to any other single-player solve. Future leaderboard/history UI will read `player_daily_completions`.
...
- The recipient experience should be direct: open a short `/s/{puzzleCode}/{time}` share link, see a compact challenge/result page, then tap to play the same puzzle. Daily share links preserve daily metadata for UI/modal continuity, and the server grants daily credit when the completed puzzle is currently one of today's active daily assignments.
...
- The home page hides public lobby browsing when no active public lobbies are available. When one or more are joinable, a "Join multiplayer game" section with the Material Groups icon appears below Quick Play and above Have a code?. Each public lobby row shows the host's room username, mode, and current puzzle difficulty; room codes stay in the direct code-entry flow rather than being the browse-list label. The list refreshes regularly while the user sits on the home page, so new public rooms can appear and stale rooms can age out.
