...
**Multiplayer lobby invite sharing is implemented and ready for manual product testing** ([DECISIONS #0053](DECISIONS.md), [#0054](DECISIONS.md)): room links stay short at `/r/{code}` and still join directly, while lobby creation moves to public-by-default, the creator gets an automatic clipboard copy plus snackbar, the copy button uses the iOS share icon, Lobby Settings is collapsed/read-only for non-hosts, and room links get lobby-specific OG images. The home page lists only public lobby-status rooms with a recently seen confirmed participant, so empty lobbies age out quickly and started games disappear from public browsing while direct links keep working. Unused warm preloaded rooms remain private until consumed. The updated `create-room` and `return-to-lobby` Edge Functions are deployed, and migration `0027` is applied on the linked Supabase project. The web changes ship through the normal `main` push / Vercel flow. `/share-preview` includes direct lobby OG-image examples.
