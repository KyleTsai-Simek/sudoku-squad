...
**Multiplayer lobby invite sharing is implemented and ready for manual product testing** ([DECISIONS #0053](DECISIONS.md)): room links stay short at `/r/{code}` and still join directly, while lobby creation moves to public-by-default, the creator gets an automatic clipboard copy plus snackbar, the copy button uses the iOS share icon, Lobby Settings is collapsed/read-only for non-hosts, and room links get lobby-specific OG images. Unused warm preloaded rooms remain private until consumed. The updated `create-room` Edge Function is deployed; the web changes ship through the normal `main` push / Vercel flow. `/share-preview` includes direct lobby OG-image examples.
