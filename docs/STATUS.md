...
**Last updated:** 2026-06-26. **Theme refresh + dark mode is implemented and verified in web; user acceptance is the remaining final step** ([DECISIONS #0044](DECISIONS.md), [#0045](DECISIONS.md), [THEME_AND_DARK_MODE_PLAN.md](THEME_AND_DARK_MODE_PLAN.md)): the app moved from hard-coded Tailwind palette utilities to semantic theme tokens, introduced an accessible default high-contrast blue primary palette, supports `auto` / `light` / `dark` with a local account-menu override, keeps notes mode warm amber, and now maps player identity colors through light/dark player tokens instead of raw inline hex values. Verification is green: web typecheck, lint (with the known `lobby-client.tsx` hook warning), build, Playwright 5 / 5, and desktop/mobile browser checks for Light/Dark/reload behavior. Final user acceptance should include an `auto` check against the native system setting. **Phase 5 authenticated accounts are built in this branch and the backend pieces are deployed to the linked Supabase project** ([DECISIONS #0043](DECISIONS.md)): migrations 0018 (mutable `issued_usernames` with `base`/`discriminator`) and 0019 (`get_completion_stats`) are live; `claim-username`, `set-username`, and `merge-progress` are deployed. Account hardening is underway: `pnpm --filter @sudoku-squad/ingest verify:accounts` now verifies Phase 5 columns/RPC, fresh anonymous sign-in, username claim, anonymous rename rejection, generated saved-account sessions, signed-in rename, username collision/freeing behavior, invalid/permanent-source merge rejection, and anonymous-completion merge union against live Supabase; `auth-store` keeps failed progress merges retryable from the Account menu instead of silently discarding the source token. Manual production testing confirmed token-hash magic-link sign-in end-to-end plus username change after an earlier PKCE `{{ .ConfirmationURL }}` failure; OTP entry, sign-out/re-sign-in, and cross-device merge still need manual/e2e coverage. Remaining Phase 5 work is tracked in [SAVED_ACCOUNTS_PLAN.md](SAVED_ACCOUNTS_PLAN.md): Supabase email template/redirect cleanup, full e2e verification of link-in-place email sign-in, real email delivery/callback behavior, sign-out/re-sign-in, and cross-device product testing. The puzzle bank is 100% QQWing-generated after the [#0042](DECISIONS.md) cutover, with metadata columns from migration 0016 and the old Kaggle upper tiers cleared by 0017. Core is now **82 / 82** after notes-faithful undo/redo and username discriminator tests. Local Playwright is **5 / 5** (SP solve, SP resume, two battle specs, coop notes-sync regression).
...
- Planning and implementation tracker are captured in [THEME_AND_DARK_MODE_PLAN.md](THEME_AND_DARK_MODE_PLAN.md), with the architecture/design choice recorded in [DECISIONS #0044](DECISIONS.md).
- `apps/web` now has Tailwind semantic color tokens backed by light/dark CSS variables, `darkMode: 'class'`, and an early root script that applies the stored theme before app paint.
- The account menu includes an `Auto` / `Light` / `Dark` selector. `auto` follows `prefers-color-scheme`; manual choices persist in `localStorage`.
- Player identity colors remain server-assigned stable slots, but web maps the stored hexes through dedicated light/dark player tokens for lobby, battle, and coop surfaces ([DECISIONS #0045](DECISIONS.md)).
- Home, lobby, game screens, boards, number pads, overlays, sheets, progress bars, and loading/error states now use semantic colors. Verification is green; final acceptance requires manual user confirmation.
