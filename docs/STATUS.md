...
- **Edge Functions** in `supabase/functions/` (deployed to the linked project):
  ...
  - `confirm-room-presence({room_id})` — marks a durable `room_players` seat visible after the client remains in the room for a few seconds ([#0050](DECISIONS.md)); pending deploy with migration 0026.
