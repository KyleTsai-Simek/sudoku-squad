...
**End-game share links are implemented with short URLs** ([DECISIONS #0052](DECISIONS.md)): completion modals now create anonymous `/s/{puzzleCode}/{time}` links, where elapsed time is base36 seconds and daily shares carry `?d=YYYY-MM-DD`. Share text is the short "Try this easy puzzle. I finished in x:xx!" form, native share avoids duplicate URLs in iMessage, and share pages preserve daily metadata into `/play/{code}?daily=...&dailyDifficulty=...` so shared daily solves record like normal daily entry. The result-specific OG image uses the refreshed playful board-card design with black Sudoku Squad branding, puzzle info, a finished-time badge, and a primary "Try this puzzle" button. `/share-preview` lists sample pages and direct OG image URLs so links can be tested without beating a puzzle. `SHARE_TOKEN_SECRET` is no longer required for new share links.
**Multiplayer lobby invite sharing is implemented and ready for manual product testing** ([DECISIONS #0053](DECISIONS.md), [#0054](DECISIONS.md), [#0055](DECISIONS.md)): room links stay short at `/r/{code}` and still join directly, while lobby creation moves to public-by-default, the creator gets an automatic clipboard copy plus snackbar, the copy button uses the iOS share icon, Lobby Settings is collapsed/read-only for non-hosts, and room links get lobby-specific OG images. The home page lists only public lobby-status rooms with a recently seen confirmed participant, so empty lobbies age out quickly and started games disappear from public browsing while direct links keep working; when lobbies are available, the list appears below Quick Play and above Have a code?, with rows showing host username and current difficulty instead of room code and lobby status. Unused warm preloaded rooms remain private until consumed. The updated `create-room` and `return-to-lobby` Edge Functions are deployed, and migrations `0027` and `0028` are applied on the linked Supabase project. The web changes ship through the normal `main` push / Vercel flow. `/share-preview` includes direct lobby OG-image examples.
...
- **`apps/web`** — Next.js 15 + React 19 + Tailwind 3.
  ...
  - Home flow: mode-first state machine in `home-client.tsx` — picks Single-player / Co-op / Battle first, then either the difficulty list (SP) or the warmed-room multiplayer path. `lib/preloaded-rooms.ts` privately preloads one co-op room and one battle room after home load, so tapping either mode can navigate immediately when the background request has finished. Public lobby browsing appears only when active public lobbies exist, refreshes with Realtime plus polling, and shows host username + current difficulty below Quick Play. The bottom of the home page shows the `total_completions` leaderboard via `get_completion_leaderboard`, highlighting the current player if ranked. See [DECISIONS #0035](DECISIONS.md), [#0048](DECISIONS.md), [#0049](DECISIONS.md), and [#0055](DECISIONS.md).
...
| Playwright e2e | `pnpm --filter @sudoku-squad/web test:e2e -- sp-resume.spec.ts` | 3 / 3 focused SP resume suite |
...
pnpm --filter @sudoku-squad/web test:e2e                  # expect full local Playwright suite; see verified table for current count
