...
- **`apps/web`** — Next.js 15 + React 19 + Tailwind 3.
  ...
  - Home flow: mode-first state machine in `home-client.tsx` — picks Single-player / Co-op / Battle first, then either the difficulty list (SP) or the Create + Join browser (multiplayer). The bottom of the home page shows the `total_completions` leaderboard via `get_completion_leaderboard`, highlighting the current player if ranked. See [DECISIONS #0035](DECISIONS.md) and [#0048](DECISIONS.md).
