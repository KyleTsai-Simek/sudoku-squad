...
**Last updated:** 2026-06-26. **Phase 5 authenticated accounts are built in this branch and the backend pieces are deployed to the linked Supabase project** ([DECISIONS #0043](DECISIONS.md)): migrations 0018 (mutable `issued_usernames` with `base`/`discriminator`) and 0019 (`get_completion_stats`) are live; `claim-username`, `set-username`, and `merge-progress` are deployed. A live smoke verified fresh anonymous `claim-username` succeeds and anonymous `set-username` is rejected. Client pieces are in place: `lib/auth-store.ts`, `/auth/callback`, `AuthSheet`, `UsernameSheet`, and a Material Symbols account menu in the root layout. Remaining Phase 5 work is full e2e verification of link-in-place email sign-in, existing-account progress merge, rename collision/freeing behavior, and sign-out/re-sign-in. The puzzle bank is 100% QQWing-generated after the [#0042](DECISIONS.md) cutover, with metadata columns from migration 0016 and the old Kaggle upper tiers cleared by 0017. Core is now **82 / 82** after notes-faithful undo/redo and username discriminator tests. Local Playwright is **5 / 5** (SP solve, SP resume, two battle specs, coop notes-sync regression).
...
**Branch:** `main` for live deploys; current account work is on `feat/phase5-accounts` until merged.
