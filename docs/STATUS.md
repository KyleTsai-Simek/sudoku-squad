...
- **Tooling:**
  ...
  - Playwright smokes in `apps/web/e2e/`: `single-player.spec.ts` navigates to `/play/3santv` (bundled sample, no Supabase needed), solves it via the keyboard, and asserts the "You won!" overlay; `sp-resume.spec.ts` fills cells, reloads, and asserts the in-progress game auto-resumes from localStorage (also bundled-sample / CI-safe); `battle.spec.ts` is a two-context smoke (create + join + start + lobby→game routing + opponent-progress Realtime broadcast + a fill→undo→redo progress-sync check per [#0039](DECISIONS.md)) that **only runs locally** — it needs live Supabase env and is skipped in CI.
