...
### Theme refresh + dark mode 🔄 (planned)

- Planning docs are captured in [THEME_AND_DARK_MODE_PLAN.md](THEME_AND_DARK_MODE_PLAN.md), with the architecture/design choice recorded in [DECISIONS #0044](DECISIONS.md).
- Code audit found the current visual system is hard-coded Tailwind palette utilities, no semantic token layer, no CSS-variable theme definitions, and a fixed light body. The board components already use explicit color lookups for state precedence and should keep that shape during the migration.
- Implementation will add semantic tokens, a high-contrast blue primary palette, light/dark CSS variables, and a local `auto` / `light` / `dark` setting. Final acceptance requires manual user confirmation.
