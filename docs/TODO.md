...
- [x] Confirmed lobby presence hardening 2026-06-27 ([DECISIONS #0050](DECISIONS.md)): migration `0026` adds `room_players.lobby_confirmed_at` / `last_seen_at`, `confirm-room-presence` marks visible participants after a short delay, `start-game` counts only confirmed rows, and stale unconfirmed lobby rows are pruned opportunistically. Pending live backend deploy until STATUS is updated.
