...
- [x] Confirmed lobby presence hardening 2026-06-27 ([DECISIONS #0050](DECISIONS.md)): migration `0026` adds `room_players.lobby_confirmed_at` / `last_seen_at`, `confirm-room-presence` marks visible participants after a short delay, `start-game` counts only confirmed rows, and stale unconfirmed lobby rows are pruned opportunistically. Migration and Edge Functions are deployed.
- [x] Presence-filtered public lobby browsing 2026-06-29 ([DECISIONS #0054](DECISIONS.md), [#0055](DECISIONS.md)): migration `0027` adds `get_public_lobbies`, the home list only shows public lobby-status rooms with recent confirmed presence, and started games disappear from public browsing while direct `/r/{code}` links continue to join. Migration `0028` extends the browse read model so available public lobby rows show host username and current puzzle difficulty below Quick Play, above Have a code?.
...
- [x] Lobby player list now shows only confirmed participants to other players, while the joining user sees themselves during the 5-second confirmation wait. This prevents mobile in-app browser hops from showing as duplicate anonymous players ([#0050](DECISIONS.md)).
...
## End-game share links + OG images 🔄 Implemented; external unfurl validation remains
...
- [x] Confirm product decisions: anonymous shares, all end-game modals, short conventional `/s/{puzzleCode}/{time}` links, daily metadata in query params, softer "Try this puzzle" copy, and playful board-card OG images.
- [x] Add share-result helpers for time formatting, share-message formatting, short URL creation/parsing, and Web Share API + clipboard fallback.
- [x] Add a public share/challenge route that renders dynamic metadata and routes humans to `/play/{puzzleCode}` for the same puzzle, preserving daily query params when present.
...
- [x] **Single-player task-switch pause.** Landed 2026-06-29 ([DECISIONS #0057](DECISIONS.md)): when `/play/[code]` is hidden for at least 5 seconds and returns, the local game enters a paused state, freezes elapsed from the hide moment, blocks input, and requires Resume. The existing one-slot `localStorage` snapshot remains the source of truth for in-progress single-player/daily state; no server-side single-player state was added.
- [x] **Co-op all-away timer pause.** Landed 2026-06-29 ([DECISIONS #0057](DECISIONS.md)): migration `0030` adds server-owned active elapsed fields on `rooms` plus gameplay presence flags on `room_players`; `update_coop_timer_presence` pauses when active-player count reaches zero and resumes when at least one player is active. The co-op UI uses the shared Resume overlay after a 5-second return-from-hidden threshold. Battle stays wall-clock after Start.
